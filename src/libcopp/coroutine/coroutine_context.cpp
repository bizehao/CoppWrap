...
struct libcopp_internal_api_set {
  ...
  static void coroutine_context_callback(LIBCOPP_COPP_NAMESPACE_ID::fcontext::transfer_t src_ctx) {
    ...
    // jump back to caller
    ins_ptr->yield([ins_ptr]() {
        if (ins_ptr->last_callback_) {
            ins_ptr->last_callback_();
        }
    });
  }
};
...
LIBCOPP_COPP_API int coroutine_context::yield(void **priv_data) LIBCOPP_MACRO_NOEXCEPT { return yield({}, priv_data); }

 LIBCOPP_COPP_API int coroutine_context::yield(std::function<void()> yield_callback,
  ...
  if (yield_callback) {
    yield_callback_ = yield_callback;
  }
